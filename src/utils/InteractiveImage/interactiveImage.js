...
const loadInteractiveImage = ref => {
  var inArea,
    interactiveImageMap = $(ref),
    captions = {
      paul: [
    ...
    initial_opts = {
      ...
      onMouseover: function(data) {
        inArea = true;
        $("#image-caption-header").text(captions[data.key][0]);
        $("#image-caption-text").text(captions[data.key][1]);
        $("#image-caption").show();
      },
  ...
  interactiveImageMap
    ...
    .bind("mouseout", function() {
      if (!inArea) {
        interactiveImageMap.mapster("set", false, "all");
      }
    });
};
export default loadInteractiveImage;
